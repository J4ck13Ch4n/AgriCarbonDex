...
const voiceInputButton = document.getElementById("voice-input-button");
const messageInputForVoice = document.querySelector(".message-input"); 
const API_URL = "http://localhost:3000"; // Đảm bảo URL này đúng
console.log("Using API_URL:", API_URL);


// +++ START: THINKING PROCESS LOGIC (1/3) - Lấy các phần tử DOM cần thiết +++
const chatbotPopup = document.querySelector(".chatbot-popup");
const toggleThoughtsBtn = document.getElementById("toggle-thoughts-btn");
const thinkingProcessContainer = document.getElementById("thinking-process");
const thinkingLog = document.getElementById("thinking-log");
const hideThinkingProcessBtn = document.getElementById("hide-thinking-process");
// +++ END: THINKING PROCESS LOGIC (1/3) +++
...
const responseFromChatbot = async (api, body, incomingMessageDiv) => {
    ...
    try {
        ...
        if (!response.ok) {
            const errorMessage = (data && data.error) || `Server responded with status ${response.status}`;
            throw new Error(errorMessage);
        }
        
        console.log("Data from server:", data); 

        // === FIX HERE: THAY ĐỔI DUY NHẤT Ở ĐÂY ===
        // Backend trả về `thinking_steps` dưới dạng HTML, nên chúng ta dùng innerHTML để hiển thị
        if (data.thinking_steps) {
            // Đổi từ .textContent sang .innerHTML
            thinkingLog.innerHTML = data.thinking_steps; 
        } else {
            // Giữ nguyên .textContent cho thông báo lỗi đơn giản
            thinkingLog.textContent = "Agent did not provide thinking steps for this response.";
        }
        // === HẾT PHẦN SỬA ===
        
        // --- Logic xử lý câu trả lời chính (KHÔNG THAY ĐỔI) ---
        let displayMessage; 
        const responseObject = data.answer; 
        ...
        // Sử dụng Marked.js để render Markdown
        messageElement.innerHTML = marked.parse(displayMessage.replaceAll('**', '').trim());

    } catch (error) {
        console.error("Error in responseFromChatbot:", error);
        messageElement.innerText = `Lỗi: ${error.message}`;
        messageElement.style.color = "#ff0000";
        // Cập nhật thinking log nếu có lỗi
        if (thinkingLog) {
            thinkingLog.textContent = `An error occurred during the request: ${error.message}`;
        }
    } finally {
        incomingMessageDiv.classList.remove("thinking");
        chatBody.scrollTo({ top: chatBody.scrollHeight, behavior: "smooth" });
...
const handleOutgoingMessage = (e) => {
    ...
    // +++ START: THINKING PROCESS LOGIC (3/3) - Hiển thị khu vực thinking khi bắt đầu hỏi +++
    chatbotPopup.classList.add("show-thinking-process"); // Thêm class để CSS hoạt động
    thinkingLog.textContent = "Agent is analyzing the request and preparing to work..."; // Thông báo ban đầu
    // +++ END: THINKING PROCESS LOGIC (3/3) +++
    ...
    setTimeout(() => {
        ...
        if (imageFileForThisMessage) {
            ...
            // Thay đổi ở đây: không còn truyền userId vì nó không được dùng trong API `/ask`
            responseFromChatbot("ask", JSON.stringify({ 
                question: textForThisMessage
            }), incomingMessageDiv);
            requestMade = true;
...
sendMessageButton.addEventListener("click", handleOutgoingMessage); // Đã sửa để gọi handleOutgoingMessage trực tiếp
chatbotToggler.addEventListener("click", () => document.body.classList.toggle("show-chatbot"));
closeToggler.addEventListener("click", () => document.body.classList.remove("show-chatbot"));
document.querySelector("#file-upload").addEventListener("click", () => fileInput.click());
document.querySelector("#emoji-picker").addEventListener("click", (e) => {
...
// +++ START: THINKING PROCESS LOGIC (Thêm sự kiện cho các nút điều khiển) +++
if (toggleThoughtsBtn) {
    toggleThoughtsBtn.addEventListener("click", () => {
        chatbotPopup.classList.toggle("show-thinking-process");
    });
}
if (hideThinkingProcessBtn) {
    hideThinkingProcessBtn.addEventListener("click", () => {
        chatbotPopup.classList.remove("show-thinking-process");
    });
}
// +++ END: THINKING PROCESS LOGIC +++
// Thay đổi dòng này:
